...
def evaluate_fold(y_true, y_pred, y_score=None):
    ...
    try:
        metrics["precision_pos"] = float(precision_score(y_true, y_pred, zero_division=0))
    except Exception:
...
def majority_baseline_predict(train_y, test_X):
    ...
    return np.full(shape=(test_X.shape[0],), fill_value=majority_label, dtype=int), np.full(shape=(test_X.shape[0],), fill_value=float(majority_label))


def run_baselines_louo(df: pd.DataFrame, feature_cols: List[str], group_col: str = "participantId", target_col: str = "High_Load") -> Dict[str, Any]:
    """
    ...
    for train_idx, test_idx in logo.split(X, y, groups):
        ...
        try:
            ...
            y_score_lr = np.full_like(y_test, fill_value=float(np.round(np.mean(y_train))))
            lr_m = evaluate_fold(y_test, y_pred_lr, y_score_lr)
...
if __name__ == "__main__":
    ...
    res["majority_fold_metrics"].to_csv(os.path.join(args.outdir, "baseline_majority_fold_metrics.csv"), index=False)
    res["logreg_fold_metrics"].to_csv(os.path.join(args.outdir, "baseline_logreg_fold_metrics.csv"), index=False)
    pd.DataFrame([res["majority_summary"]]).to_csv(os.path.join(args.outdir, "baseline_majority_summary.csv"), index=False)
    pd.DataFrame([res["logreg_summary"]]).to_csv(os.path.join(args.outdir, "baseline_logreg_summary.csv"), index=False)
