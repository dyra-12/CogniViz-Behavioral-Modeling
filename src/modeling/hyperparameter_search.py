...
def run_grouped_grid_search(df, feature_cols, group_col="participantId", n_jobs=1, random_state=2025):
    ...
    grid = GridSearchCV(pipe, param_grid, cv=logo.split(X, y, groups), scoring="f1", n_jobs=n_jobs, verbose=1)
    grid.fit(X, y)
